...
router.post('/', protect, async (req, res) => {
    try {
        // Enforce plan restrictions here
        const plan = req.user.subscription?.plan || 'basic';
        const { getMaxWorkers } = require('../utils/planUtils');
        const max = getMaxWorkers(plan);

        if (max === 0) {
            return res.status(403).json({ message: 'Your plan does not allow adding workers. Upgrade to enable this.' });
        }

        const currentWorkers = await User.countDocuments({ employerId: req.user.ownerId, role: 'worker' });
        if (currentWorkers >= max) {
            return res.status(403).json({ message: `Your plan allows up to ${max} workers. Please upgrade to add more.` });
        }
