            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '1.5rem'}}>
                {filteredHistory.length === 0 ? (
                    ...
                    filteredHistory.map((log) => {
                        ...
                        if (log.actionType === 'JOB_SHEET_CREATE') {
                            const metadata = log.metadata || {};
                            const sheetId = metadata.jobSheetId;
                            return (
                                <div key={log._id} className="glass-card" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '1rem 1.5rem', gap: '1rem', flexWrap: 'wrap'}}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        {metadata.customerPhone && (
                                            <div className="text-secondary" style={{fontSize: '0.85rem', marginTop: '4px'}}>
                                                Customer Phone: {metadata.customerPhone}
                                            </div>
                                        )}
                                        {(metadata.productType || metadata.productInfo) && (
                                            <div className="text-secondary" style={{fontSize: '0.85rem', marginTop: '2px'}}>
                                                Device: {metadata.productType || ''} {metadata.productInfo ? `- ${metadata.productInfo}` : ''} {metadata.productImei ? `(IMEI: ${metadata.productImei})` : ''}
                                            </div>
                                        )}
                                        {metadata.deviceIssue && (
                                            <div style={{marginTop: '0.5rem', padding: '0.5rem 0.8rem', borderRadius: '6px', background: 'rgba(255,255,255,0.05)', fontSize: '0.85rem'}}>
                                                <strong>Reported Problem:</strong> {metadata.deviceIssue}
                                            </div>
                                        )}
                                        <div className="text-secondary" style={{fontSize: '0.8rem', marginTop: '0.6rem'}}>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                        {sheetId && (
                                            <button 
                                                onClick={() => window.open(`/job-sheet-view/${sheetId}`, '_blank')} 
                                                className="btn btn-secondary" 
                                                style={{padding: '0.3rem 0.7rem', fontSize: '0.8rem', display: 'inline-flex', alignItems: 'center', gap: '0.3rem'}}
                                            >
                                                📄 View Job Sheet
                                            </button>
                                        )}
                                        {user?.role !== 'worker' && (
                                            <button onClick={() => deleteLog(log._id)} className="btn btn-red" style={{padding: '0.3rem 0.7rem', fontSize: '0.8rem'}}>
                                                Delete
                                            </button>
                                        )}
                                    </div>
