    ...
    return (
        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)', gap: '1rem' }}>
            ...
            <div className="pos-layout" style={{ flex: 1, overflow: 'hidden' }}>
                {/* Contacts Sidebar */}
                <div className="glass-card" style={{ flex: '1 1 320px', maxWidth: '100%', display: 'flex', flexDirection: 'column', padding: '1.5rem' }}>
                    <form onSubmit={addPerson} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <select className="form-input" style={{flex: 1, padding: '0.5rem', background: newPersonType === 'receive' ? 'rgba(127, 168, 146, 0.12)' : 'rgba(196, 132, 122, 0.12)', border: `1px solid ${newPersonType === 'receive' ? 'var(--ok-green)' : 'var(--ok-red)'}`}} value={newPersonType} onChange={e=>setNewPersonType(e.target.value)}>
                        ...
                        <>
                                ...
                                {activeMessages.length === 0 ? (
                                    ...
                                    activeMessages.map(m => (
                                        <div key={m._id} style={{
                                            ...
                                            border: `1px solid ${m.type === 'receive' ? 'rgba(127, 168, 146, 0.3)' : 'rgba(196, 132, 122, 0.3)'}`,
                                            borderBottomRightRadius: m.type === 'receive' ? '0' : '16px',
                                            borderBottomLeftRadius: m.type === 'receive' ? '16px' : '0',
                                            padding: '1rem 1.2rem', borderRadius: '16px', minWidth: '150px', maxWidth: '100%',
                            ...
                            <div style={{ padding: '1.5rem', borderTop: '1px solid var(--border-color)', background: 'rgba(255,255,255,0.02)' }}>
                                <form onSubmit={sendTransaction} style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
                                    <select className="form-input" style={{ flex: 1, minWidth: '120px', background: type === 'receive' ? 'var(--ok-green-glow)' : 'var(--ok-red-glow)', border: `1px solid ${type === 'receive' ? 'var(--ok-green)' : 'var(--ok-red)'}`, color: '#fff' }} value={type} onChange={e=>setType(e.target.value)}>
