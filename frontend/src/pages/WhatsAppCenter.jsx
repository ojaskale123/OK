  ...
  return (
    <div className="animate-fade-in" style={{ padding: '1rem' }}>
        ...
        <div className="glass-card">
            ...
            {jobs.length === 0 ? (
                <p className="text-secondary">No recent jobs available for messaging.</p>
            ) : (
                    ...
                    {jobs.map(job => (
                        <div key={job._id} style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                <button onClick={() => sendWhatsAppUpdate(job, 'collected')} className="btn btn-secondary" style={{ fontSize: '0.8rem', padding: '0.4rem 0.8rem' }}>
                                    <Send size={14}/> Collected Msg
                                </button>
                                <button onClick={() => sendWhatsAppUpdate(job, 'status')} className="btn btn-secondary" style={{ fontSize: '0.8rem', padding: '0.4rem 0.8rem' }}>
                                    <Send size={14}/> Status Msg
                                </button>
                                <button onClick={() => sendWhatsAppUpdate(job, 'ready')} className="btn btn-primary" style={{ fontSize: '0.8rem', padding: '0.4rem 0.8rem', background: 'var(--ok-green)', color: '#000' }}>
                                    <Send size={14}/> Ready for Pickup
                                </button>
                            </div>
                        </div>
                    ))}
