...
const RepairManagement = () => {
  ...
  const [formData, setFormData] = useState({
      customerName: '', customerPhone: '', deviceModel: '', issue: '', itemNote: '', workerId: ''
  });
  ...
  const fetchJobs = async () => {
      try {
          const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://ok-ax2v.onrender.com'}/api/repairs?limit=200`, {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          if(res.ok) {
              ...
              setShowModal(false);
              setFormData({ customerName: '', customerPhone: '', deviceModel: '', issue: '', itemNote: '', workerId: '' });
              fetchJobs();
          } else {
  ...
  const sendWhatsAppUpdate = (job) => {
      ...
      if (isMobile) {
          window.location.href = `whatsapp://send?phone=91${job.customerPhone}&text=${encodeURIComponent(text)}`;
      } else {
          window.open(`https://wa.me/91${job.customerPhone}?text=${encodeURIComponent(text)}`, '_blank');
      }
  };
    ...
    <div className="animate-fade-in" style={{ padding: '1rem' }}>
        ...
        {showModal && (
            <div className="modal-overlay" style={{position: 'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.7)', zIndex: 1000, display:'flex', justifyContent:'center', alignItems:'center'}}>
                <div className="glass-card" style={{ flex: '1 1 400px', maxWidth: '100%', position: 'relative' }}>
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
                        ...
                        <textarea className="neon-input" rows="3" placeholder="Items given by customer (SIM, cover, charger, etc.)" value={formData.itemNote} onChange={e => setFormData({...formData, itemNote: e.target.value})} style={{ minHeight: '90px', resize: 'vertical' }} />
        ...
        {viewingJob && (
            <div className="modal-overlay" style={{position: 'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.9)', zIndex: 1000, display:'flex', justifyContent:'center', alignItems:'center'}}>
                <div className="glass-card" style={{ width: '100%', maxWidth: '600px', position: 'relative', padding: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                            <p className="text-secondary" style={{ fontSize: '0.85rem' }}>Items Given by Customer</p>
                            <textarea
                                className="neon-input"
                                rows="3"
                                value={viewingJob.itemNote || ''}
                                onChange={(e) => setViewingJob({ ...viewingJob, itemNote: e.target.value })}
                                onBlur={(e) => updateJob(viewingJob._id, { itemNote: e.target.value || '' })}
                                style={{ width: '100%', minHeight: '90px', resize: 'vertical', marginTop: '0.5rem' }}
                                placeholder="SIM, cover, charger, etc."
                            />
                        </div>
                    </div>
                        ...
                        jobs.map(job => (
                            <tr key={job._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                <td style={{ padding: '1rem' }}>
                                    ...
                                    {job.itemNote ? (
                                        <div style={{ fontSize: '0.78rem', color: 'var(--neon-blue)', marginTop: '0.35rem', whiteSpace: 'pre-wrap' }}>
                                            Note: {job.itemNote}
                                        </div>
                                    ) : null}
                                </td>
                                            ...
                                            {transferringJobId === job._id ? (
                                                ...
                                                <>
                                                    ...
                                                    {!job.workerId ? (
                                                        ...
                                                        }} className="btn btn-primary" style={{ padding: '0.2rem 0.5rem', fontSize: '0.75rem', background: 'rgba(212, 184, 122, 0.4)' }}>
                                                            Accept Job
                                                        </button>
                                                    ) : (
                                                        <>
                                ...
                                <td style={{ padding: '1rem' }}>
                                    <input 
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setJobs((prev) => prev.map((j) => (j._id === job._id ? { ...j, costing: val } : j)));
                                        }}
